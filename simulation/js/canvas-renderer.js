 ...
 */
    drawNF(nf) {
        const x = nf.position?.x ?? nf.x ?? 0;
        const y = nf.position?.y ?? nf.y ?? 0;
    ...
    drawConnection(conn) {
        ...
        const sourceX = (sourceNF.position?.x ?? sourceNF.x ?? 0) + 20;
        const sourceY = (sourceNF.position?.y ?? sourceNF.y ?? 0) + 20;
        const targetX = (targetNF.position?.x ?? targetNF.x ?? 0) + 20;
        const targetY = (targetNF.position?.y ?? targetNF.y ?? 0) + 20;
        ...
        // Interface labels removed for cleaner visual - kept only in logs
    }
    ...
    handleMouseDown(e) {
        ...
        for (const nf of allNFs) {
            if (this.isPointInNF(x, y, nf)) {
                const nfX = nf.position?.x ?? nf.x ?? 0;
                const nfY = nf.position?.y ?? nf.y ?? 0;
                this.isDragging = true;
                this.draggedNF = nf;
                this.dragOffset = { x: x - nfX, y: y - nfY };
    ...
    isPointInNF(x, y, nf) {
        ...
        const width = 40;
        const height = 40;
        return x >= nfX && x <= nfX + width && y >= nfY && y <= nfY + height;
    }
 ...
 */
    ...
    drawNFBusConnection(connection) {
        ...
        const nfCenterX = (nf.position?.x ?? nf.x ?? 0) + 20;
        const nfCenterY = (nf.position?.y ?? nf.y ?? 0) + 20;
